...
const cors = require("cors") //trazendo pacote cors - consumir api no front end
const conectaDatabase = require("./database") //linkando o doc Database.js
conectaDatabase() //chamando a função que conecta o banco de dados
const Mulher = require("./mulherModel")

const app = express() //iniciando o app
app.use(express.json()) //config requisição no formato json
app.use(cors())
...
async function criaMulher(request, response) {
    ...
    try{
        ...
        response.status(201).json(mulherCriada) //código do protocolo http - 201 resposta criada
    } catch (erro) {
        console.log(erro)
    }
}
